...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class ConnectAndJoinRandom : MonoBehaviourPunCallbacks
    {
        ...
        {
            Debug.Log("ConnectAndJoinRandom.cs: ConnectNow() will now call: PhotonNetwork.ConnectUsingSettings().");
        ...
        {
            Debug.Log("ConnectAndJoinRandom.cs: OnConnectedToMaster() was called by PUN. This client is now connected to Master Server in region [" + PhotonNetwork.CloudRegion +
        ...
        {
            Debug.Log("ConnectAndJoinRandom.cs: OnJoinedLobby(). This client is now connected to Relay in region [" + PhotonNetwork.CloudRegion + "]. This script now calls: PhotonNetwork.JoinRandomRoom();");
            PhotonNetwork.JoinRandomRoom();
        }

        public override void OnJoinRandomFailed(short returnCode, string message)
        {
            Debug.Log("ConnectAndJoinRandom.cs: OnJoinRandomFailed() was called by PUN. No random room available in region [" + PhotonNetwork.CloudRegion + "], so we create one. Calling: PhotonNetwork.CreateRoom(null, new RoomOptions() {maxPlayers = 4}, null);");
        ...
        {
            Debug.Log("ConnectAndJoinRandom.cs: OnDisconnected(" + cause + ")");
        }

        public override void OnJoinedRoom()
        {
            Debug.Log("ConnectAndJoinRandom.cs: OnJoinedRoom() called by PUN. Now this client is in a room in region [" + PhotonNetwork.CloudRegion + "]. Game is now running.");
        }
    }
